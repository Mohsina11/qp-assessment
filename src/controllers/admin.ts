...
class AdminController {
  async addItem(req: Request, res: Response) {
    try {
      ...
      if (!name || !unitprice) {
        return res.status(400).json({ success: false, error: 'Name and unit price are required.' });
      }
      ...
      return res.status(201).json({
        success: true, message: 'Grocery item added successfully', data: {
          id: groceryData?.groceryId
        }
      });
    } catch (error:any) {
      if (error?.code === 'ER_DUP_ENTRY') {
        return res.status(400).json({ success: false, error: 'Grocery Item with name already exists.' });
      } else {
  ...
  async removeItem(req: Request, res: Response) {
    try {
      ...
      if (isNaN(groceryId) || groceryId <= 0) {
        return res.status(400).json({ success: false, error: 'Invalid grocery Id' });
      }
      ...
      return res.json({ success: true, message: 'Grocery item removed successfully' });
    } catch (error) {
  ...
  async updateItem(req: Request, res: Response) {
    try {
      ...
      const groceryItemToUpdate = await groceryRepository.findOne({
        where:
          { groceryId: groceryId, deleted: false }
      });
      ...
      return res.json({ success: true, message: 'Grocery item updated successfully' });
    } catch (error) {
  ...
  async manageInventory(req: Request, res: Response) {
    ...
    return res.json({ success: true, message: 'Grocery item updated successfully',data:{
